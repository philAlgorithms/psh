...
char *get_location(char *command)
{
	...
	while (dirs)
	{
		temp = malloc(_stringlen(dirs->dir) + _stringlen(command) + 2);
		if (!temp)
			return (NULL);

		_stringcpy(temp, dirs->dir);
		_stringcat(temp, "/");
...
char *fill_path_dir(char *path)
{
	...
	for (i = 0; path[i]; i++)
	{
		...
		{
			_stringncat(path_copy, &path[i], 1);
		}
	}
	return (path_copy);
}
